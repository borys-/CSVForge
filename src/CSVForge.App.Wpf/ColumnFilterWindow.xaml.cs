...
public partial class ColumnFilterWindow : Window
{
    ...
    public ColumnFilterWindow(string columnName, IReadOnlyList<ColumnValueOption> values, IReadOnlyList<string?>? selectedValues)
    {
        InitializeComponent();
    ...
    public IReadOnlyList<string?> SelectedValues => _valueCheckBoxes.Where(item => item.CheckBox.IsChecked == true).Select(item => item.Value).ToArray();
    ...
    private void SearchTextBox_TextChanged(object sender, TextChangedEventArgs e)
    {
        ...
        {
            item.CheckBox.Visibility = search.Length == 0 || DisplayValue(item.Value).Contains(search, StringComparison.CurrentCultureIgnoreCase)
    ...
    private void ClearFilter_Click(object sender, RoutedEventArgs e) { ClearFilter = true; DialogResult = true; }
    private void Apply_Click(object sender, RoutedEventArgs e) => DialogResult = true;
    private static string DisplayValue(string? value) => value is null ? "(NULL)" : value.Length == 0 ? "(Puste)" : value;
