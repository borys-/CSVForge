...
public static class CliApplication
{
            ...
            case "import":
                {
                    ...
                    ImportResult result = await services.GetRequiredService<IImportCsvUseCase>()
                        .ExecuteAsync(new ImportRequest(file, name, !options.ContainsKey("no-header"), Delimiter(options), options.GetValueOrDefault("encoding"), IntOption(options, "batch-size", 100_000), !options.ContainsKey("no-header")));
    ...
    static void PrintHelp()
    {
        ...
        import --workspace <db> --file <plik.csv> [--name <nazwa>] [--delimiter ;] [--batch-size 100000] [--no-header]
